...
// Import Models
const SaveChatsWa = require("../Models/ChatsWA");
const MemoryWA = require("../Models/MemoryWA");

// Functions
const { sendMessage } = require("./Whatsapp-Send-Message");
const { SaveChatsWA } = require("./SaveChatsWADB");
const { saveMemory } = require("./SaveMemoryWADB");
...
exports.Saam = async (id, userNumber) => {
  // Getting User's Memory Data
  console.log("Getting User's Memory Data");
  const memory = await MemoryWA.findOne({ userNumber: id });
  const memoryMsg = memory["messages"];
  // Removing _id property
  const rolesAndContent = memoryMsg.map(({ role, content }) => ({
    role,
    content,
  }));

  // Load Memory Messages to the Prompt
  ...
  console.log(`SAAM Response: ${completion.data.choices[0].message.content}`);

  //Send Saam Response
  await sendMessage(completion.data.choices[0].message.content, userNumber);

  // Save AI's mesage/response to chatDB
  await SaveChatsWA(id, "Ai", completion.data.choices[0].message.content);

  // Save AI's Message/Response to MemoryDB
  await saveMemory(id, "assistant", completion.data.choices[0].message.content);
};
